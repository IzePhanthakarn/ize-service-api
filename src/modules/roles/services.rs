...
fn check_super_admin(pool: &DbPool, role_id: Uuid) -> Result<(), (StatusCode, ErrorResponse)> {
...
pub async fn create_role(pool: &DbPool, user_role_id: Uuid, req: CreateRoleRequest) -> Result<RoleResponse, (StatusCode, ErrorResponse)> {
    // 1. ตรวจสอบสิทธิ์ก่อนเลย!
    check_super_admin(pool, user_role_id)?;

    let mut conn = pool.get().unwrap();
    let new_role = NewRole { name: req.name.to_lowercase() }; // บังคับให้ชื่อ Role เป็นตัวเล็กทั้งหมดเพื่อความเป๊ะ
...
pub async fn get_all_roles(pool: &DbPool, user_role_id: Uuid) -> Result<Vec<RoleResponse>, (StatusCode, ErrorResponse)> {
    check_super_admin(pool, user_role_id)?;
